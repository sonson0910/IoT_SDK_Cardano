...
namespace cardano_iot
{
    namespace security
    {
        struct Encryption::Impl
        {
            ...
            EncryptedData mock_aes_encrypt(const std::vector<uint8_t>& plaintext, const EncryptionKey& key) {
                EncryptedData result;
                result.algorithm = EncryptionAlgorithm::AES_256_GCM;
                result.nonce = generate_secure_random(12); // GCM nonce
                ...
                for (size_t i = 0; i < plaintext.size(); ++i) {
                    result.ciphertext[i] = plaintext[i] ^ key.key_data[i % key.key_data.size()];
                }

                // Mock authentication tag
                result.auth_tag = generate_secure_random(16);

                return result;
            }

            // Mock AES-256-GCM decryption
            std::vector<uint8_t> mock_aes_decrypt(const EncryptedData& encrypted_data, const EncryptionKey& key) {
                ...
                for (size_t i = 0; i < encrypted_data.ciphertext.size(); ++i) {
                    plaintext[i] = encrypted_data.ciphertext[i] ^ key.key_data[i % key.key_data.size()];
                }
        ...
        EncryptedData Encryption::encrypt(const std::vector<uint8_t>& plaintext, const std::string& key_id)
        {
            ...
            switch (key->algorithm) {
                case EncryptionAlgorithm::AES_256_GCM:
                case EncryptionAlgorithm::AES_256_CBC:
                    result = pimpl_->mock_aes_encrypt(plaintext, *key);
                    break;
                
                case EncryptionAlgorithm::CHACHA20_POLY1305:
                case EncryptionAlgorithm::XCHACHA20_POLY1305:
                    result = pimpl_->mock_aes_encrypt(plaintext, *key); // Mock with same function
        ...
        std::vector<uint8_t> Encryption::decrypt(const EncryptedData& encrypted_data)
        {
            ...
            switch (encrypted_data.algorithm) {
                ...
                case EncryptionAlgorithm::CHACHA20_POLY1305:
                case EncryptionAlgorithm::XCHACHA20_POLY1305:
                    result = pimpl_->mock_aes_decrypt(encrypted_data, *key);
                    break;
        ...
        EncryptedData Encryption::encrypt_with_password(const std::vector<uint8_t>& plaintext, const std::string& password)
        {
            ...
            auto result = pimpl_->mock_aes_encrypt(plaintext, temp_key);
        ...
        std::vector<uint8_t> Encryption::decrypt_with_password(const EncryptedData& encrypted_data, const std::string& password)
        {
            ...
            auto result = pimpl_->mock_aes_decrypt(encrypted_data, temp_key);
